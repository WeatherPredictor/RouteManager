package com.zookeeper.delegate.forecast;
...
@Path("/forecast")
public class DelegateRunForecast 
{

	private static Logger logger = Logger.getLogger(DelegateRunForecast.class);
	...
	public String delegate() throws UnknownHostException, IOException 
	{
		...
		ServiceProvider<Void> serviceProvider = serviceDiscovery.serviceProviderBuilder().serviceName("runForecast")
				.build();
		try 
		{
		...
		try 
		{
	        ...
	        int thisIndex = RunForecastService.getIndex();
	        RunForecastService.setIndex(thisIndex+1);
