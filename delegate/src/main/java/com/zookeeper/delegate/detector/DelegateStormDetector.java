...
public class DelegateStormDetector 
{
	...
	private static Logger logger = Logger.getLogger(DelegateStormDetector.class);
	...
	public String delegate() throws UnknownHostException, IOException 
	{
		...
		ServiceProvider<Void> serviceProvider = serviceDiscovery.serviceProviderBuilder().serviceName("stormDetector")
				.build();
		try 
		{
		...
		try 
		{
	        ...
	        int thisIndex = StormDetectorService.getIndex();
	        StormDetectorService.setIndex(thisIndex+1);
