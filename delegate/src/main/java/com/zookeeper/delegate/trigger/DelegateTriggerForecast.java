package com.zookeeper.delegate.trigger;
...
@Path("/trigger")
public class DelegateTriggerForecast 
{

	private static Logger logger = Logger.getLogger(DelegateTriggerForecast.class);
	...
	public String delegate() throws UnknownHostException, IOException 
	{
		...
		ServiceProvider<Void> serviceProvider = serviceDiscovery.serviceProviderBuilder().serviceName("triggerForecast")
				.build();
		try 
		{
		...
		try 
		{
	        ...
	        int thisIndex = TriggerForecastService.getIndex();
	        TriggerForecastService.setIndex(thisIndex+1);
